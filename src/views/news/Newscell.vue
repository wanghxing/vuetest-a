<template>
  <section class="financial-list">
    <section class="collect" @click="jumpPage(newsInfor.arcId, newsInfor.title)">
      <aside>
        ...
        <section class="Cleft clearfix">
          <img
            ...
            style="width:16px;height:10px;margin-top:3px;"
          />
          <span class="fl">{{ newsInfor.author_name }}</span>
        </section>
        <section class="Cright clearfix">
          <img
            class="fl"
            src="../../assets/images/clock.png"
            style="width:14px;height:14px;"
          />
          <span class="fl">{{ newsInfor.date | getYMD }}</span>
        </section>
        <div style="clear: both"></div>
      </aside>
      <aside>
        <img :src="newsInfor.thumbnail_pic_s" style="border-radius: 4px;" />
      </aside>
      <div style="clear: both"></div>
    </section>
  </section>
...
export default {
  ...
  computed: {},
  methods: {
    jumpPage(arcId, title) {
      // window.location.href = this.newsInfor.url
      this.$router.push({
        name: "news-detail",
        query: { id: arcId, title: title }
      });
    }
  }
};
